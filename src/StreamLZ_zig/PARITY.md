...
- [~] **32. F3** — PARTIAL. New module `encode/high_optimal_parser.zig` lands the DP parser scaffolding: `updateState`, `updateStatesZ`, `collectStatistics` (greedy seed pass with lazy-1/lazy-2 + recent-offset dedupe), and the top-level `optimal` entry point with full forward-DP Phase 1 loop (state band initialization, per-position match extraction from the MLS match table, SC boundary filter, recent-offset + new-offset candidate propagation, long-match-skip optimization). **What's NOT yet ported**: Phase 2 backward extraction (state-chain walk → token array), Phase 3 per-chunk stats update, outer-loop re-run for L8+, and end-to-end validation against C# reference output. The current code compiles + test-passes via `return error.BailOut` after the forward pass. Finishing F3 is the remaining blocker for step 34 (D9 wiring L6-L11 into `compressFramed`). Roughly 200-300 more Zig lines needed. 225/225 tests pass.
