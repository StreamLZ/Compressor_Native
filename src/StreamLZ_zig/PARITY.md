...
- [~] **34. D9** — PARTIAL. Level range in `compressFramed` extended from 1-5 to 1-11; unified L6-L11 divert through a new `compressFramedHigh` path. Helper: `high_compressor.doCompress` dispatches L1-L4 through `high_fast_parser.compressFast` and L5+ through `high_optimal_parser.optimal`, with a tagged-union `HighHasher` (`MatchHasher1/2x/4/4Dual/none`) so the comptime-generic fast parser can be called from a runtime-level branch. `compressFramedHigh` writes a frame header with `codec=high` + mapped codec level (L6→5, L7→7, L8→9, L9→5, L10→7, L11→9), allocates an MLS over the whole source, runs the appropriate match finder (BT4 for codec level 9, hash-based otherwise), then iterates 256 KB blocks with a standard 2-byte block header + 4-byte chunk header + sub-chunk loop. Each sub-chunk calls `doCompress`; raw-fallback and block-level cost-bail mirror the Fast path. **Known gap**: compressed-output roundtrips for L9/L11 currently fail inside the decoder with an `integer overflow` in `readDistanceCore` at `bit_reader.zig:184` — the High encoder's offset-stream emission in `assembleCompressedOutput` does not yet match the decoder's offset reader on the trial inputs used in this session. Needs byte-level diffing vs C# reference output. The scaffolding roundtrips cleanly for tiny inputs that fall through the uncompressed frame-block fallback (tested with "Hello, world!" at L6). No SC prefix table emission yet — L6-L8 are currently encoded as non-SC. Three commits' worth of follow-up work: (a) debug encoder/decoder byte parity, (b) add SC prefix table, (c) wire `extractLaoFromMls` round-start offsets through MLS correctly so the optimal parser sees the matches. 230/230 unit tests pass (including the tiny L6 roundtrip and the L1-L5 fixtures which remain untouched).
